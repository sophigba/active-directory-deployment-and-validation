...
* Connectivity: Confirms that the attacker VM can reach the Target-PC via the network.

* RDP Availability: Ensures the RDP service is correctly configured and listening for external requests.

* Telemetry Generation: Forces the generation of high-volume Event ID 4625 (Failed Logon) logs, providing the necessary data to test the Splunk ingestion pipeline.
